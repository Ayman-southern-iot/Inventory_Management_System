...
export class ApproverSlotsService {
  ...
  async set(input: SetApproverSlotInput, actorId: string): Promise<ApproverSlot[]> {
  ...
  async resolveForDepartment(departmentId: string | null, slotCount: number): Promise<string[]> {
    const rows = await this.db
      .selectFrom('approver_slots')
      .select(['department_id', 'slot_no', 'user_id'])
      .where('slot_no', '<=', slotCount)
      .where((eb) =>
        departmentId === null
          ? eb('department_id', 'is', null)
          : eb.or([eb('department_id', '=', departmentId), eb('department_id', 'is', null)]),
      )
      ...
      .orderBy(sql`department_id NULLS FIRST`)
      .execute();
    ...
    for (let slotNo = 1; slotNo <= slotCount; slotNo += 1) {
      ...
      if (!userId) {
        throw new ConflictError(
          `Approver slot ${slotNo} is not assigned. An administrator must set it before requisitions can be submitted.`,
        );
      }
