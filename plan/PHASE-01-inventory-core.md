...
- [x] **1.1 Categories** — tree, `is_trackable` flag (laptops and R&D hardware on; furniture off,
...
- [x] **1.2 Products** — `product_code` (the Storage ID), name, category, unit,
...
- [x] **1.3 Locations** — `storage_zones` ("Meta", "Nvidia") → `storage_compartments` ("1A", "3C"),
...
- [x] **1.4 Placements + ledger** — `stock_placements` with `CHECK (quantity >= 0)` and
...
- [x] **1.5 StockService** — the only writer. `receive`, `move`, `reserve`, `release`, `issue`,
...
- [x] **1.6 Partial move / split** — move N of M from one compartment to another; source row is
...
- [x] **1.7 IM inventory UI** — product list with trigram search, product card showing total plus
...
- [x] **1.8 Concurrency + invariant tests** — N simultaneous reserves against quantity 1: exactly
