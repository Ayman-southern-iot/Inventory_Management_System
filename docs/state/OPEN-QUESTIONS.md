...
| G-18 | Integration tests write real uploads into apps/api/storage/files because the test env does not override FILE_STORAGE_DIR. 686 files there against 1 stored_files row in dev — the rest are orphaned test artefacts that nothing cleans up. | Harmless in production (tests do not run there) but it inflates every dev backup and hides genuine orphans. | Phase 06 |
