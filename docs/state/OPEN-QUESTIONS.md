...
| G-20 | ✅ | **Eight reversal tests were `it.skip` while the harness could not vary config per spec.** | **Closed 2026-09-02.** `createTestApp()` now takes a `CONFIG` override, so a spec that needs a flag-gated feature builds its app under a different policy instead of the flag being flipped for the whole run. Additive — without the argument the behaviour is byte-identical, so the other spec files are untouched. All eight are running again, and the same mechanism keeps the approved-amount-revision tests alive. **The integration suite has zero skipped tests.** | Closed |
