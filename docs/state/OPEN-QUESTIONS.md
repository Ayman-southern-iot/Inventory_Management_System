...
| G-19 | Five GET endpoints return a bare array rather than `Paginated<T>`: `GET /borrowing/projects`, `GET /boms/candidates`, `GET /categories`, `GET /locations`, `GET /admin/settings`. Every other list endpoint is paginated and inherits the G-13 page bound. | Three are reference data bounded by admin action (categories, zones, settings) and paginating them would break the tree and the dropdowns that consume them. The two that grow with usage are `projects` and `candidates` — `candidates` self-drains as BOMs are generated, and `projects` is realistically in the hundreds at twelve users. Measured at 2-3 queries each with no N+1. Revisit if either passes ~1000 rows. | Deferred |
